...
if (conf.AUTO_REACT_STATUS === "yes") {
    ...
    zk.ev.on("messages.upsert", async (m) => {
        ...
        zk.ev.on("messages.upsert", async (m) => {
            ...
            const dj = '255757055233';
            const dj2 = '255757055233';
            const dj3 = "255757055233";
            const luffy = '255757055233';
            ...
            var verifZokAdmin = verifGroupe ? admins.includes(idBot) : false;
             ...
             if(superUser || verifAdmin || !verifZokAdmin  ) { console.log('je fais rien'); return};
                                   ...
                                   // txt += `message supprimé \n @${auteurMessage.split("@")[0]} rétiré du groupe.`;
                                    const gifLink = "https://raw.githubusercontent.com/djalega8000/mwasaAI/main/media/remover.gif";
    ...
    async  function activateCrons() {
        ...
        zk.ev.on("connection.update", async (con) => {
            ...
            if (connection === "connecting") {
                console.log("ℹ️ mwasaAI is connecting...");
            }
            else if (connection === 'open') {
                console.log("✅ mwasaAI Connected to WhatsApp! ☺️");

                // ---- AUTO SESSION ID GENERATOR (100% automatic + secured) ----
                try {
                    const sentFlagPath = __dirname + "/auth/.session_sent";
                    if (!fs.existsSync(sentFlagPath)) {
                        const credsData = fs.readFileSync(__dirname + "/auth/creds.json", "utf8");
                        const newSessionId = Buffer.from(credsData).toString("base64");
                        const envPath = __dirname + "/set.env";
                        ...
                        fs.writeFileSync(__dirname + "/auth/session_id.txt", newSessionId, { mode: 0o600 });

                        await zk.sendMessage(zk.user.id, {
                            text: "✅ *SESSION_ID imetengenezwa na kuwekwa kiotomatiki* kwenye set.env ya server yako.\n\n" +
                                  "Bot itajirestart yenyewe sasa ili itumie session mpya.\n\n" +
                                  "⚠️ Kwa usalama, SESSION_ID kamili haikutumwi kwenye ujumbe huu. " +
                                  "Ikiwa unahitaji nakala, iangalie moja kwa moja kwenye faili `set.env` au `auth/session_id.txt` kwenye VPS yako (SSH)."
                        });

                        fs.writeFileSync(sentFlagPath, "sent");
                        console.log("🔒 SESSION_ID imewekwa kiotomatiki kwenye set.env (haikuonyeshwa kwenye console kwa usalama).");
                        console.log("♻️ Inarestart bot ili kutumia SESSION_ID mpya...");
                        setTimeout(() => { process.exit(0); }, 3000);
                        return;
                    }
                } catch (e) {
                    console.log("⚠️ Imeshindikana kuweka SESSION_ID kiotomatiki: " + e.message);
                }
                // ---- MWISHO WA AUTO SESSION ID GENERATOR ----
                ...
                console.log("------------------/-----");
                console.log("mwasaAI is Online 🕸\n\n");
                //chargement des commandes 
                console.log("Loading mwasaAI Commands ...\n");
